"""Trains BootEALinker on OpenEA's EN-FR-15K-V1 dataset using its native
train/test/validation split and reports Hits@1, Hits@10, MRR via
linkingtk.eval.Evaluator.evaluate_ranked.

Train pairs seed the main structural triple set (via pseudo-triples, not a
separate loss); validation pairs drive early stopping; test pairs are held
out and only used to score ranked predictions -- this measures genuine
generalization to entities the model wasn't directly told to align. Every
sub_epoch epochs, an editing-based bootstrapping round finds additional
high-confidence matches (via max-weight bipartite matching over a
similarity-and-top-k-filtered candidate set) and retrains a separate
alignment loss over the accumulated labeled-alignment set's pseudo-triples.
Negative sampling for the main structural loss is "truncated" -- restricted
to each entity's own current nearest-neighbor set, refreshed every
sub_epoch epochs. Ranking is exhaustive (every test-source entity against
every test-target entity, via linkingtk.eval.rank_exhaustive, no
blocking/candidate restriction), matching OpenEA's own evaluation
methodology -- see docs/examples/ea_kge_benchmarks.md for methodology
details.
...
Run with: `uv run python examples/bootea_benchmark.py`
"""
...
def main() -> None:
    ...
    linker = BootEALinker(num_epochs=2000)  # embedding_dim=100, batch_size=5000
    linker.fit(
        entities1,
        entities2,
