"""Benchmark: RSN4EALinker trained on a DBP15K/OpenEA-style native
...
milestone's per-method acceptance criteria (#34).

Train pairs seed the alias-expanded graph that sampled walks train over;
test pairs are held out and only used to score ranked predictions -- this
checks a genuine generalization signal, not just whether training recovers
what it was directly taught (contrast test_rsn4ea.py's fully-seeded
pipeline-correctness tests). Ranking is exhaustive (via
linkingtk.eval.rank_exhaustive, no blocking), matching OpenEA's own
evaluation methodology. See examples/rsn4ea_benchmark.py for the same
...
def test_rsn4ea_linker_reports_ranked_metrics_on_held_out_split(tmp_path: Path) -> None:
