"""Benchmark: BootEALinker trained on a DBP15K/OpenEA-style native
...
milestone's per-method acceptance criteria (#30).

Train pairs seed the shared embedding table (via pseudo-triples); test
...
recovers what it was directly taught (contrast test_bootea.py's
fully-seeded pipeline-correctness tests). Ranking is exhaustive (via
linkingtk.eval.rank_exhaustive, no blocking), matching OpenEA's own
evaluation methodology. See examples/bootea_benchmark.py for the same
...
def test_bootea_linker_reports_ranked_metrics_on_held_out_split(tmp_path: Path) -> None:
