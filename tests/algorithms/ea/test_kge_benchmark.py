"""Benchmark: KGELinker trained on a DBP15K/OpenEA-style native train/test
split, evaluated with linkingtk.eval.Evaluator.evaluate_ranked (Hits@1,
Hits@10, MRR) -- the Knowledge Graph Embeddings milestone's acceptance
criteria (#14).

Train pairs are given to fit() as seed alignment triples (bridging the
two KGs' graphs); test pairs are held out and only used to score ranked
predictions -- this checks a genuine generalization signal, not just
whether training recovers what it was directly taught (contrast
test_kge.py's fully-seeded pipeline-correctness tests). See
examples/kge_benchmark.py for the same methodology at real dataset scale.
"""
...
class _BenchmarkDataset(_KGZipDataset):
    """Test-only concrete subclass pointed at the benchmark fixture."""
...
class _AllPairs(BlockingStrategy):
    """Lets every pair through -- see test_kge.py's identical helper."""
...
def test_kge_linker_reports_ranked_metrics_on_held_out_split(tmp_path: Path) -> None:
    ...
    linker = KGELinker(embedding_dim=16, num_epochs=100, learning_rate=0.01)
