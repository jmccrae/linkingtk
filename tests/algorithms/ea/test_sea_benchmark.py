"""Benchmark: SEALinker trained on a DBP15K/OpenEA-style native
...
milestone's per-method acceptance criteria (#33).

Train pairs seed the mapping loss; test pairs are held out and only used
to score ranked predictions -- this checks a genuine generalization signal,
not just whether training recovers what it was directly taught (contrast
test_sea.py's fully-seeded pipeline-correctness tests). Ranking is
exhaustive (via linkingtk.eval.rank_exhaustive, no blocking), matching
OpenEA's own evaluation methodology. See examples/sea_benchmark.py for the
same methodology at real dataset scale.
"""
...
def test_sea_linker_reports_ranked_metrics_on_held_out_split(tmp_path: Path) -> None:
