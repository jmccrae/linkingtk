...
# benchmark. See test_sea_benchmark.py for held-out generalization.
...
class TestFitAndLink:
    def test_recovers_seeded_alignment(self) -> None:
        linker = SEALinker(embedding_dim=16, num_epochs=300, batch_size=32)
    ...
    def test_early_stopping_runs_without_error(self) -> None:
        ...
        linker = SEALinker(embedding_dim=16, num_epochs=100, batch_size=32)
        linker.fit(
            _KG1,
            _KG2,
            _GROUND_TRUTH,
    ...
    def test_source_embedding_differs_from_target_embedding(self) -> None:
        # SEA projects the source side through mapping_mat_1 but scores
        # the target side raw -- source_embedding/target_embedding must
        # preserve that asymmetry for rank_exhaustive to score correctly.
        linker = SEALinker(embedding_dim=16, num_epochs=50, batch_size=32)
        linker.fit(_KG1, _KG2, _GROUND_TRUTH, graph=_GRAPH, random_state=0)
    ...
    def test_recovers_seeded_alignment_on_cuda(self) -> None:
        linker = SEALinker(embedding_dim=16, num_epochs=300, batch_size=32, device="cuda")
        linker.fit(_KG1, _KG2, _GROUND_TRUTH, graph=_GRAPH, random_state=0)
...
class TestErrors:
    ...
    def test_link_with_entity_absent_from_training_graph_raises(self) -> None:
        linker = SEALinker(embedding_dim=8, num_epochs=5, batch_size=32)
        linker.fit(_KG1, _KG2, _GROUND_TRUTH, graph=_GRAPH, random_state=0)
